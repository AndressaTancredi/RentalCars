...
feature 'Admin search for client' do
  scenario 'from name' do
    user = User.create!(email: 'test@test.com', password: '12345678')
    Client.create!(name: 'Ana Maria', email: 'ana@email.com', document: '18885510205' )
    Client.create!(name: 'Andressa Tancredi', email: 'andressa@email.com', document: '32336954540' )


    login_as(user, scope: :user)
    visit root_path
    click_on 'Clientes'
    fill_in 'Busca', with: 'Andressa'
    click_on 'Buscar'

    expect(current_path).to eq search_clients_path
    expect(page).to have_content 'Andressa Tancredi'
    expect(page).to have_content 'andressa@email.com'

    expect(page).not_to have_content 'Ana Tancredi'
  ...
  scenario 'from partial name with multiple results' do
    # Arrange
    user = User.create!(email: 'test@test.com', password: '12345678')
    Client.create!(name: 'Ana Silva', email: 'ana@email.com',
                     document: '18885510205')
    Client.create!(name: 'André Silva', email: 'andre@email.com',
                     document: '32336954540')
    Client.create!(name: 'Carlos Oliveira', email: 'carlos@email.com',
                     document: '71534235167')
